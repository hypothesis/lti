...
class SharedLTISetupTests(object):
    ...
    def test_if_theres_no_oauth_consumer_key_it_logs_an_error_and_crashes(
            self, pyramid_request, log):
        del pyramid_request.POST[constants.OAUTH_CONSUMER_KEY]
...
class TestLTISetupWhenWeDontHaveAnAccessToken(SharedLTISetupTests):
    ...
    def pyramid_request(self, pyramid_request):
        pyramid_request.auth_data.get_lti_token.return_value = None
...
class TestLTISetupWhenWeHaveAValidAccessToken(SharedLTISetupTests):
    ...
    def test_it_returns_a_document_chooser_page_if_theres_no_assignment_type(
        ...
        response = launch.lti_setup(pyramid_request)
...
@pytest.fixture
def log(patch):
    return patch('lti.views.launch.log')


@pytest.fixture
def pack_state(patch):
    return patch('lti.views.launch.util.pack_state')


@pytest.fixture
def simple_response(patch):
    return patch('lti.views.launch.util.simple_response')


@pytest.fixture
def requests(patch):
    return patch('lti.views.launch.requests')
...
@pytest.fixture
def oauth(patch):
    return patch('lti.views.launch.oauth')


@pytest.fixture
def pdf(patch):
    return patch('lti.views.launch.pdf')


@pytest.fixture
def web(patch):
    return patch('lti.views.launch.web')
